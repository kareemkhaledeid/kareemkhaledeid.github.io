...
/* Final editorial system: one visual language across every section. */
...
.section-heading h2,.archive-group h3,.capability h3,.project-meta h3,.case-copy h2,.about-copy h2,.book-copy h2,.contact h2{font-weight:500}
.eyebrow{font-size:10px!important;line-height:1.2!important}.section-heading>p,.archive-group>p,.case-copy>p,.capability p,.about-copy>p,.book-copy>p,.contact-grid>div>p{line-height:1.8}
.project-meta h3,.selected-meta h3{font-weight:500}.project-meta p,.selected-meta p{line-height:1.55}
.nav-links a,.button,.text-link,.filter{transition:opacity .2s ease,color .2s ease,border-color .2s ease,background .2s ease,transform .2s ease}
.portfolio-archive .archive-photo{cursor:zoom-in}
/* Conversion layer */
...
.section-intro{max-width:680px;color:#888;font-size:13px;line-height:1.85}.archive-note{max-width:700px;line-height:1.8}
.case-copy .text-link{display:inline-block;margin-top:3px}.capability h3{margin-top:50px}
@media(max-width:700px){.lightbox{padding:18px}.lightbox p{left:18px;bottom:12px;font-size:8px}.hero-availability{margin-top:16px}.section-heading>p{max-width:620px}}
`;
...
/* Put the story in the order a recruiter or client actually reads it. */
const capabilities=document.querySelector('#capabilities');
const exhibition=document.querySelector('#exhibition');
const archive=document.querySelector('#archive');
if(workSection&&exhibition)workSection.after(exhibition);
if(exhibition&&capabilities)exhibition.after(capabilities);
if(experience&&archive)experience.after(archive);

/* Add one clear availability signal without changing the source portfolio copy. */
