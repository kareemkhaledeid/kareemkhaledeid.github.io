...
if(heroCopy){let availability=heroCopy.querySelector('.hero-availability');if(!availability){availability=document.createElement('div');availability.className='hero-availability';heroCopy.querySelector('.hero-actions')?.after(availability);}availability.innerHTML='<i aria-hidden="true"></i><span>Available for projects worldwide</span>';}

/* Clean the existing Visual Language section shown in the portfolio: remove image bands and keep typography only. */
const visualLanguage=[...document.querySelectorAll('section,div')].find(el=>{const t=(el.textContent||'').replace(/\s+/g,' ').trim();return t.includes('VISUAL LANGUAGE')&&t.includes('From stage light')&&t.includes('THEATRE')&&t.includes('EXHIBITIONS');});
if(visualLanguage){
  visualLanguage.classList.add('visual-language-clean');
  visualLanguage.querySelectorAll('img,picture,.gallery-card,[class*="gallery"],[style*="background-image"]').forEach(el=>{el.style.setProperty('background-image','none','important');if(el.tagName==='IMG'||el.tagName==='PICTURE')el.style.setProperty('display','none','important');});
  visualLanguage.querySelectorAll('*').forEach(el=>{if(el.style.backgroundImage)el.style.setProperty('background-image','none','important');});
  const candidates=[...visualLanguage.querySelectorAll('a,li,div,span,p')];
  const labels=candidates.filter(el=>/^(THEATRE|SCENOGRAPHY|EVENTS|EXHIBITIONS)$/i.test(el.textContent.trim()));
  if(labels.length===4){const list=document.createElement('div');list.className='visual-language-list';labels.forEach(el=>{const item=document.createElement('div');item.textContent=el.textContent.trim();list.appendChild(item);el.replaceWith(document.createComment('visual-language-item'));});const anchor=visualLanguage.querySelector('p:last-of-type')||visualLanguage.lastElementChild;anchor?.after(list);}
}
